...
def main():
    ...
    while True:
        ...
        if j == 0:  # 如果是第一个数
            ...
            r = (20 - passed_coordinates[j][4])/ 0.001 if (20 - passed_coordinates[j][4]) / 0.001 < (25 - passed_coordinates[j][5]) / 0.001 else (25 - passed_coordinates[j][
                5]) / 0.001
            flag = 0
        elif j % 2 == 0:  # 偶数为垂直
            r = (15 - passed_coordinates[j][5])/ 0.001 if ((15 - passed_coordinates[j][5])/ 0.001) < (25 - passed_coordinates[j][4]) / 0.001 else (25 - passed_coordinates[j][
                4]) / 0.001
            flag = 1
        ...
        if j == 1 :
            ...
            if judge_candidate() and coordinates[min_i][6]:
                wrong_candidates.append([min_i, coordinates[min_i]])
                passed_coordinates[j][5] = min(passed_coordinates[j - 1][5] + pre_toNow_dist * 0.001,5)
                passed_coordinates[j][4] = passed_coordinates[j - 1][4] + pre_toNow_dist * 0.001
            else:
                passed_coordinates[j][4] = passed_coordinates[j - 1][4] + pre_toNow_dist * 0.001
            ...
            if judge_candidate() and coordinates[min_i][6]:
                wrong_candidates.append([min_i, coordinates[min_i]])
                passed_coordinates[j][4] = min(passed_coordinates[j - 1][4] + pre_toNow_dist * 0.001, 5)
                passed_coordinates[j][5] = passed_coordinates[j - 1][5] + pre_toNow_dist * 0.001
            else:
                passed_coordinates[j][5] = passed_coordinates[j - 1][5] + pre_toNow_dist * 0.001
                passed_coordinates[j][4] = 0

        print("{0}【----当前飞过第{1}号校正点----】{2}".format(j, min_i, passed_coordinates[j]))
